...
(function () {
  ...
  function initLenis() {
    ...
    // anchor links → lenis (skip checkout triggers — they have their own handler)
    document.querySelectorAll('a[href^="#"]').forEach((a) => {
      a.addEventListener("click", (e) => {
        // if this element opens the checkout, let initCheckout handle it
  ...
  function initCheckout() {
    ...
    function open() {
      ...
      // stop Lenis smooth scroll so body doesn't scroll behind modal
      if (lenis) lenis.stop();
      startPixTimer();
      // NOTE: do NOT set opacity via GSAP — the CSS transition handles it.
      // Setting it inline via gsap breaks the CSS class-based close transition.
    }
    function close() {
      ...
      stopPixTimer();
      // reset to initial state after transition completes (matches CSS 0.35s)
    ...
    function resetModal() {
      ...
      const confettiCanvas = document.getElementById("coConfetti");
      if (confettiCanvas) {
    ...
    // Use closest() instead of e.target === overlay to handle cases where
    // the confetti canvas or other absolute children absorb the pointer event
